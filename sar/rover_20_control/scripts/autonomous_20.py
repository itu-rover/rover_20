...
class autonomous():
	...
	def find_largest_space(self, data = None):
		...
		if self.turn_to_empty_space and largest_space is not None:
			...
			self.turn_to_yaw(angle, angle_error_threshold = math.pi / self.yaw_angle_error_divider)

		return largest_space

	#https://www.youtube.com/watch?v=oCXfMJnGWaA

	def sign_of_value(self, value):
	...
	def find_fitting_space(self, data = None):
		try:
			...
			if self.encoders_working:
				angle_sign = self.sign_of_value(self.fix_yaw_angle(math.atan2((self.target_y - self.curr_y), (self.target_x - self.curr_x))))
			else:
				angle_sign = self.sign_of_value(self.fix_yaw_angle(math.atan2((self.target_lat - self.curr_lat), (self.target_lon - self.curr_lon))))

			reverse = True if angle_sign > 0 else False

			empty_spaces = self.find_empty_spaces(data = data) #Find empty spaces from latest scan

			empty_spaces.sort(key = lambda space: space.iter_start, reverse = reverse) #Saga donecekken kucukten buyuge
			...
			for empty_space in empty_spaces:
				...
				#Check if vehicle can fit in the space
	...
	def go_x_meters(self, speed, meters):
		...
		rate = rospy.Rate(10)
		self.twist.linear.x = speed
		while self.curr_x * self.sign_of_value(speed) < initial_dist + meters * self.sign_of_value(speed):
			self.twist_pub.publish(self.twist)
	...
	def get_dist_xy(self, targetx = 0, targety = 0):
		...
		if targetx == 0 and targety == 0:
			...
			self.dist = float(math.sqrt(math.pow((self.target_x - self.curr_x), 2) + math.pow((self.target_y - self.curr_y), 2)))
	...
	def turn_to_yaw(self, target_yaw, angle_error_threshold = None):
		...
		print("Current yaw: {}\nTurning to: {}".format(self.curr_yaw, target_yaw))
		...
		while (yaw_dist > angle_error_threshold) or (yaw_dist < -1 * angle_error_threshold):
			...
			self.curr_yaw = self.fix_yaw_angle(self.curr_yaw)
			yaw_dist = self.curr_yaw - target_yaw
			#print("Current distance to target: {}".format(self.dist))
	...
	def obstacle_avoidance(self, turn_sec = 0.4):
	...
	def autonomous_drive(self):
		...
		for i in range(500):
			...
			if float(self.dist) < float(self.goal_reach_thresh):
				print("Goal reached!\nCurrent x: {} Current y: {}".format(self.curr_x, self.curr_y))
				self.reset_twist()
				break
