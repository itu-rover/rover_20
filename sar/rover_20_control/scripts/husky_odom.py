...
class Localization(object):
	def __init__(self):
		...
		self.dist_btw_wheels = 0.55 #0.85 for rover // 0.55 for husky
	...
	def serial_callback(self,data):
		...
		self.splitted = self.encoder_data.split(",")
		if(self.splitted[0] == 'A'):
			...
			if(float(self.splitted[3]) >= 1000):
				self.back_left = (-(float(self.splitted[3])-1000)) * self.surrounding_of_wheel / 60
			if(float(self.splitted[3]) < 1000):
				self.back_left = (float(self.splitted[3])) * self.surrounding_of_wheel / 60

			if(float(self.splitted[2]) >= 1000):
				self.front_right = (-(float(self.splitted[2])-1000)) * self.surrounding_of_wheel / 60
			if(float(self.splitted[2]) < 1000):
				self.front_right = (float(self.splitted[2])) * self.surrounding_of_wheel / 60
	...
	def jointstate_cb(self, data):
		[self.front_left, self.front_right, self.back_left, self.back_right] = [vel * 0.165 for vel in data.velocity]
	...
	def controller(self):
		...
		while not rospy.is_shutdown():
			...
			print(str(self.front_left) + ", " + str(self.back_left) + ", " + str(self.front_right) + ", " + str(self.back_right))
			#print("delta_x: {}, delta_y: {}, changed: {}\n".format(self.delta_x, self.delta_y, changed))
			#print("th: " + str(math.degrees(self.curr_yaw - self.init_yaw)) + ", deltath: " + str(math.degrees(self.delta_th)))
...
class Localization(object):
	def __init__(self):
		...
		self.surrounding_of_wheel = 0.15 * math.pi * 2 #0.15 for rover // 0.165 for husky
		self.reduction = 100
		self.counter = 0
		self.curr_yaw = 0
		self.front_left = 0
