...
def callback(data):
    ...
    pxi=data.axes[0]*0.03
    pyi=data.axes[1]*0.03
    pzi=data.axes[5]*0.03
...
def Euler_Inverse(Px,Py,Pz,x,y,z):
	...
	p2x=px-l*math.sin(z)
	p2y=py-l*math.cos(z)
	p2z=pz-L3_4*math.sin(y)

	r=math.sqrt(pow(p2x,2)+pow(p2y,2))
    
	alpha=math.atan2((p2z-L0_1),r)
	Omega=math.atan2(L2_3*math.sin(t2),L1_2+L2_3*math.cos(t2))
 	D=(pow(p2x,2)+pow(p2y,2)+pow(p2z-L0_1,2)-pow(L1_2,2)-pow(L2_3,2))/(2*L1_2*L2_3)

 	t0=math.atan2(p2y,p2x)-math.pi/4
