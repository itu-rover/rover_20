...
class arm_serial(object):
	def __init__(self):
		...
		self.joint3 = 40                  #initial degree value of joint3
		self.joint4 = 40
		self.joint5 = 40
		self.joint6 = 40
		...
		self.joint1_serial = "060"
		self.joint2_serial = "060"        #initial serial for joint2
		self.joint3_serial = "060"        #initial serial for joint3
		self.joint4_serial = "060"
		self.joint5_serial = "060"
		self.joint6_serial = "060"
	...
	def joy_callback(self, data):
		...
		if data.buttons[5]== 1 and data.buttons[3]== 1 :                                       #Open gripper
		...
		elif data.buttons[2]==1 and data.axes[6]!=0 :                                           #Increase/decrease joint1.

			self.joint1 = self.joint1 + int(data.axes[6])
			...
			self.joint1_serial = floattostring(self.elk_control_1)
		...
		elif data.buttons[3]==1 and data.axes[7]!=0 :                                        #Increase/decrease joint2.

			self.joint2 = self.joint2 + int(data.axes[7])
			...
			self.joint2_serial = floattostring(self.elk_control_2)


			self.new_message_2 = True


		elif data.buttons[1]==1 and data.axes[7]!=0 :                                           #Increase/decrease joint3.

			self.joint3 = self.joint3 + int(data.axes[7])
			...
			self.joint3_serial = floattostring( self.elk_control_3 )


			self.new_message_3 = True


		elif data.buttons[0]==1 and data.axes[7]!=0 :                                          
			
			self.joint4 = self.joint4 + int(data.axes[7])
			...
			self.joint4_serial = floattostring(self.elk_control_4)


			self.new_message_4 = True


		elif data.axes[4]==1 and data.axes[6]!=0 :                                          
		
			self.joint5 = self.joint5 + int(data.axes[6])
			...
			self.joint5_serial = floattostring(self.elk_control_5)
			...
			self.joint6 = self.joint6 + int(data.axes[6])
			...
			self.joint6_serial = floattostring(self.elk_control_6)
	...
	def main(self):
		...
		while not rospy.is_shutdown():
			...
			self.pub.publish("S"+self.joint1_serial+self.joint2_serial+self.joint3_serial+self.joint4_serial+self.joint5_serial+self.joint6_serial+self.gripper_command+"C"+"F")
